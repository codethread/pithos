	...
	it("reads body from --body-file when provided", async () => {
		...
		await Effect.runPromise(
			Effect.provide(
				...
				Layer.mergeAll(
					dbLayer,
					makeIdServiceTest(["artifact_art_body"]),
					FsServiceLive,
					silentOutput,
				),
			),
		...
		const row = db.prepare("SELECT body FROM artifacts WHERE id = ?").get("artifact_art_body") as {
			body: string;
		};
		db.close();
	...
	it("stores empty body when --body-file is omitted", async () => {
		...
		await Effect.runPromise(
			Effect.provide(
				...
				Layer.mergeAll(
					dbLayer,
					makeIdServiceTest(["artifact_art_nobody"]),
					FsServiceLive,
					silentOutput,
				),
			),
	...
	it("inspect task includes artifacts array with added artifact", async () => {
		...
		await Effect.runPromise(
			Effect.provide(
				...
				Layer.mergeAll(
					dbLayer,
					makeIdServiceTest(["artifact_inspect1"]),
					FsServiceLive,
					silentOutput,
				),
			),
	...
	it("multiple artifacts on same task are all returned", async () => {
		...
		await Effect.runPromise(
			Effect.provide(
				...
				Layer.mergeAll(
					dbLayer,
					makeIdServiceTest(["artifact_multi1"]),
					FsServiceLive,
					silentOutput,
				),
			),
		...
		await Effect.runPromise(
			Effect.provide(
				...
				Layer.mergeAll(
					dbLayer,
					makeIdServiceTest(["artifact_multi2"]),
					FsServiceLive,
					silentOutput,
				),
			),
