...
class GraphDependencyEdgeRow extends Schema.Class<GraphDependencyEdgeRow>("GraphDependencyEdgeRow")(
	{
...
const toTaskRelationshipSummary = (row: TaskRelationshipSummaryRow): TaskRelationshipSummary => ({
	id: row.id,
...
export const findDependencyCycle = (edges: readonly DependencyEdge[]): readonly string[] | null => {
...
export const computeTaskClaimability = (
	...
	claimable:
		task.status === "queued" && unresolvedDependencyIds.length === 0 && supersededBy === null,
...
const assertClosedTaskGraph = (graph: TaskGraph): Effect.Effect<void, PithosError> =>
	Effect.gen(function* () {
		...
		for (const node of graph.nodes) {
			...
			if (node.superseded_by_task_id !== null && !nodeIds.has(node.superseded_by_task_id)) {
				yield* Effect.fail(
					new PithosError({
						code: "INTERNAL_ERROR",
...
export const loadCurrentTaskGraph = (): Effect.Effect<TaskGraph, PithosError, DbService> =>
	loadIdRows(
     ...
     ORDER BY created_at ASC, id ASC`,
	).pipe(Effect.flatMap(loadCurrentTaskGraphConnectedTaskIds), Effect.flatMap(buildTaskGraph));
