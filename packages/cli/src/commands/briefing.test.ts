...
describe("briefingCommand (unit — fake DB)", () => {
  ...
  it("places done task in Ready for review section", async () => {
    const seedRows = new Map<string, readonly DbRow[]>([
      ...
      [BRIEFING_SQL.TASKS, [makeTaskRow({ id: "task_done_1", status: "done", title: "Done task" })]],
      [BRIEFING_SQL.STALE_RUNS, []],
      [BRIEFING_SQL.ARTIFACTS, []],
    ])
    const out = makeOutputServiceTest()
  ...
  it("places dead_letter task in Needs Adam section", async () => {
    const seedRows = new Map<string, readonly DbRow[]>([
      ...
      [
        BRIEFING_SQL.TASKS,
        [makeTaskRow({ id: "task_dl_1", status: "dead_letter", title: "Dead task", attempts: 3, max_attempts: 3 })],
      ],
      [BRIEFING_SQL.STALE_RUNS, []],
      [BRIEFING_SQL.ARTIFACTS, []],
    ])
  ...
  it("places failed tasks in Stale / failed section", async () => {
    const seedRows = new Map<string, readonly DbRow[]>([
      [BRIEFING_SQL.WATERMARK, []],
      [BRIEFING_SQL.TASKS, [makeTaskRow({ id: "task_failed_1", status: "failed", title: "Failed task" })]],
      [BRIEFING_SQL.STALE_RUNS, []],
      [BRIEFING_SQL.ARTIFACTS, []],
    ])
    const out = makeOutputServiceTest()
  ...
  it("places question artifacts in Needs Adam section", async () => {
    const seedRows = new Map<string, readonly DbRow[]>([
      ...
      [
        BRIEFING_SQL.ARTIFACTS,
        [makeArtifactRow({ id: "artifact_q", kind: "question", title: "Can you clarify?", task_id: "task_abc" })],
      ],
    ])
    const out = makeOutputServiceTest()
    const layer = Layer.merge(makeDbServiceTest(seedRows), out.layer)
  ...
  it("design-brief artifact appears under done task in Ready for review", async () => {
    const seedRows = new Map<string, readonly DbRow[]>([
      [BRIEFING_SQL.WATERMARK, []],
      [BRIEFING_SQL.TASKS, [makeTaskRow({ id: "task_design", status: "done", title: "Design task" })]],
      [BRIEFING_SQL.STALE_RUNS, []],
      [
        BRIEFING_SQL.ARTIFACTS,
        [makeArtifactRow({ id: "artifact_design", kind: "design-brief", title: "Design doc", task_id: "task_design" })],
      ],
    ])
    const out = makeOutputServiceTest()
    const layer = Layer.merge(makeDbServiceTest(seedRows), out.layer)
  ...
  it("claimed task includes run reference in Active line", async () => {
    const seedRows = new Map<string, readonly DbRow[]>([
      ...
      [
        BRIEFING_SQL.TASKS,
        [makeTaskRow({ id: "task_ref_1", status: "claimed", lease_owner_run_id: "run_abc", title: "Ref task" })],
      ],
      [BRIEFING_SQL.STALE_RUNS, []],
      [BRIEFING_SQL.ARTIFACTS, []],
    ])
