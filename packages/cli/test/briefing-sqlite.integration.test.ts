...
describe("briefingCommand (integration — real SQLite)", () => {
	...
	it("shows dead_letter task in Needs Adam section", async () => {
		...
		db.prepare(
			`UPDATE tasks SET status = 'dead_letter', attempts = 3, max_attempts = 3 WHERE id = 'task_dl_br'`,
		).run();
		db.close();
	...
	it("shows active run with expired heartbeat (stale-by-age) in Stale / failed section", async () => {
		...
		const db = new Database(dbPath);
		db.prepare(
			`UPDATE runs SET last_heartbeat_at = strftime('%Y-%m-%dT%H:%M:%SZ', datetime('now', '-20 minutes')) WHERE id = 'run_hb_expired'`,
		).run();
		db.close();
	...
	it("does not show active run with fresh heartbeat in Stale / failed", async () => {
		...
		const db = new Database(dbPath);
		db.prepare(
			`UPDATE runs SET last_heartbeat_at = strftime('%Y-%m-%dT%H:%M:%SZ', datetime('now', '-5 minutes')) WHERE id = 'run_fresh_hb'`,
		).run();
		db.close();
