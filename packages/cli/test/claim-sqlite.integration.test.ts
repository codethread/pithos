...
describe("claimCommand (integration — real SQLite)", () => {
	...
	it("successfully claims the oldest queued task", async () => {
		...
		const row = db
			.prepare(
				"SELECT status, lease_owner_run_id, fencing_token, attempts FROM tasks WHERE id = 'task_claim1'",
			)
			.get() as {
			status: string;
	...
	it("sets lease_until to a future datetime", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT lease_until FROM tasks WHERE id = 'task_lease1'").get() as {
			lease_until: string;
		};
		db.close();
	...
	it("appends a task.claimed event", async () => {
		...
		const db = new Database(dbPath);
		const event = db
			.prepare("SELECT type, actor_run_id, payload_json FROM events WHERE type = 'task.claimed'")
	...
	it("claims tasks FIFO — oldest queued task is claimed first", async () => {
		...
		const db = new Database(dbPath);
		const claimed = db.prepare("SELECT id, status FROM tasks WHERE status = 'claimed'").get() as {
			id: string;
			status: string;
		};
		const queued = db.prepare("SELECT id, status FROM tasks WHERE status = 'queued'").get() as {
			id: string;
			status: string;
		};
		db.close();
	...
	it("skips an older blocked task and claims the newer ready task", async () => {
		...
		const db = new Database(dbPath);
		const blockedRow = db.prepare("SELECT status FROM tasks WHERE id = 'task_blocked'").get() as {
			status: string;
		};
		db.close();
	...
	it("treats cross-scope unfinished dependencies as not claimable", async () => {
		...
		const db = new Database(dbPath);
		const waitingRow = db.prepare("SELECT status FROM tasks WHERE id = 'task_waiting'").get() as {
			status: string;
		};
		db.close();
	...
	it("only one claim event is appended in the race scenario", async () => {
		...
		const db = new Database(dbPath);
		const events = db.prepare("SELECT type FROM events WHERE type = 'task.claimed'").all() as {
			type: string;
		}[];
		db.close();
