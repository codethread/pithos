...
const allFlags = (args: readonly string[], name: string): readonly string[] =>
	args.flatMap((a, i) => (a === name && args[i + 1] !== undefined ? [args[i + 1]!] : []));
const req = (args: readonly string[], name: string): string =>
	parseFlag(args, name) ?? fail("VALIDATION_ERROR", `missing ${name}`);
...
const body = (ctx: Ctx, args: readonly string[]): string => {
	...
	if ((inline === undefined) === (file === undefined))
		fail("VALIDATION_ERROR", "supply exactly one of --body or --body-file");
	const text = inline ?? ctx.services.fs.readText(file!);
	if (text.trim() === "") fail("VALIDATION_ERROR", "body must be non-empty");
	return text;
};
const capability = (value: string): Capability =>
	["triage", "design", "execute", "escalate"].includes(value)
		? (value as Capability)
		: fail("VALIDATION_ERROR", `invalid capability: ${value}`);
const mode = (value: string): Mode =>
	value === "afk" || value === "hitl" ? value : fail("VALIDATION_ERROR", `invalid mode: ${value}`);
const agent = (value: string): AgentKind =>
	["pdx", "pandora", "toil", "greed", "war"].includes(value)
		? (value as AgentKind)
...
export const runCli = (ctx: Ctx, argv: readonly string[]): number => {
	try {
		...
		if (a === "scope" && b === "upsert") {
			...
			if (!["global", "repo", "worktree"].includes(kind))
				fail("VALIDATION_ERROR", `invalid scope kind: ${kind}`);
			const rawPath = kind === "global" ? undefined : req(rest, "--path");
		...
		if (a === "task" && b === "enqueue") {
			const runId = resolvedRun(ctx, rest);
			const scope = req(rest, "--scope");
			const cap = capability(req(rest, "--capability"));
			const title = req(rest, "--title");
			const depends = allFlags(rest, "--depends-on");
			...
			out({ ok: true, task: { id: tid, status: "queued", scope_id: scope, capability: cap } });
			return 0;
		}
		if (a === "task" && b === "claim") {
		...
		if (a === "task" && b === "heartbeat") {
			...
			const token = parseFlag(rest, "--token");
			...
			if (task !== undefined) {
				...
				event(db, "task.heartbeat", {
					...
					payload: {
						run_id: runId,
						fencing_token: Number(token),
						previous_status: tr.status,
						status: tr.status === "claimed" ? "running" : tr.status,
					},
				});
				out({
					ok: true,
					task: { id: task, status: tr.status === "claimed" ? "running" : tr.status },
				});
				return 0;
			}
		...
		if (a === "task" && (b === "complete" || b === "fail")) {
			const taskId = c ?? fail("VALIDATION_ERROR", "missing task id");
			const runId = req(rest, "--run");
			const token = Number(req(rest, "--token"));
			const st = b === "complete" ? "done" : "failed";
			const res =
				b === "complete"
					? parseFlag(rest, "--result-file")
						? ctx.services.fs.readText(parseFlag(rest, "--result-file")!)
						: "{}"
					: JSON.stringify({ reason: req(rest, "--reason") });
			...
			out({ ok: true, task: { id: taskId, status: r.status } });
			return 0;
		}
		if (a === "task" && b === "inspect") {
			const taskId = c ?? fail("VALIDATION_ERROR", "missing task id");
			...
			out({
				...
				task: {
					...
					status: t.status,
					claimable: t.status === "queued" && unresolved.length === 0,
		...
		if (a === "graph" && b === "inspect") {
			out({
				ok: true,
				graph: {
					selector:
						parseFlag(rest, "--all") !== undefined || rest.includes("--all")
							? { kind: "all" }
							: { kind: "task", value: parseFlag(rest, "--task") ?? parseFlag(rest, "--scope") },
					nodes: [],
					edges: [],
				},
			});
		...
		if (a === "task" && b === "artifact" && c === "add") {
			...
			db.prepare(
				...
				req(rest, "--kind"),
				req(rest, "--title"),
				parseFlag(rest, "--body-file")
					? ctx.services.fs.readText(parseFlag(rest, "--body-file")!)
					: "",
			);
			out({ ok: true, artifact: { id: aid } });
			return 0;
		}
		if (a === "task" && ["supersede", "cancel"].includes(b ?? "")) {
			out({ ok: true });
			return 0;
		}
