		...
		Effect.gen(function* () {
			...
			yield* Effect.try({
				try: () => mkdirSync(dirname(dbPath), { recursive: true }),
					...
					new PithosError({
						code: "INTERNAL_ERROR",
						message: `DB mkdir error: ${e instanceof Error ? e.message : "mkdir failed"}`,
					}),
			});
				...
				SqliteClient.layer({ filename: dbPath }).pipe(
					Layer.mapError(
						(e) =>
							new PithosError({
								code: "INTERNAL_ERROR",
								message: `DB open error: ${e instanceof Error ? e.message : "open failed"}`,
							}),
					),
				),
			);
			...
			yield* sql.unsafe("PRAGMA foreign_keys = ON").pipe(Effect.mapError(wrapSqlError));
			...
			return {
				query: (sqlStr, params) =>
					...
					sql.unsafe<DbRow>(sqlStr, params as never).pipe(Effect.mapError(wrapSqlError)),

				run: (sqlStr, params) =>
					sql.unsafe(sqlStr, params as never).pipe(Effect.asVoid, Effect.mapError(wrapSqlError)),

				withTransaction: (effect) =>
					sql.withTransaction(effect).pipe(Effect.mapError(wrapSqlError)),
			};
		}),
	);
 ...
 */
export const DbServiceLive: Layer.Layer<DbService, PithosError> =
	makeDbServiceLive(resolveDbPath());
