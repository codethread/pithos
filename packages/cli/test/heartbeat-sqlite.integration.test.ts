/**
 * Integration tests for pithos heartbeatCommand — real SQLite. Unit coverage lives in src/commands/heartbeat.test.ts.
 */
...
describe("heartbeatCommand (integration — real SQLite)", () => {
	...
	const claim = async (runId: string, taskId?: string): Promise<number> => {
		...
		if (row === undefined) {
			...
			if (taskId !== undefined) {
				...
				const r = db2.prepare("SELECT fencing_token FROM tasks WHERE id = ?").get(taskId) as {
	...
	it("updates last_heartbeat_at on heartbeat", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT last_heartbeat_at FROM runs WHERE id = ?").get(runId) as {
	...
	it("advances run status from 'starting' to 'running'", async () => {
		...
		const before = db1.prepare("SELECT status FROM runs WHERE id = ?").get(runId) as {
			status: string;
		};
		db1.close();
		...
		const db2 = new Database(dbPath);
		const after = db2.prepare("SELECT status FROM runs WHERE id = ?").get(runId) as {
			status: string;
		};
		db2.close();
	...
	it("records last_hook when --hook is provided", async () => {
		...
		await Effect.runPromise(
			Effect.provide(
				heartbeatCommand({ run: runId, hook: "PreToolUse" }),
				Layer.merge(dbLayer, silentOutput),
			),
		);

		const db = new Database(dbPath);
		const row = db.prepare("SELECT last_hook FROM runs WHERE id = ?").get(runId) as {
			last_hook: string;
		};
		db.close();
	...
	it("fails NOT_FOUND when run does not exist", async () => {
		const exit = await runEff(
			Effect.provide(
				heartbeatCommand({ run: "run_nonexistent" }),
				Layer.merge(dbLayer, silentOutput),
			),
		);
	...
	it("extends lease_until when task is advanced to running", async () => {
		...
		const db1 = new Database(dbPath);
		const before = db1.prepare("SELECT lease_until FROM tasks WHERE id = ?").get(taskId) as {
			lease_until: string;
		};
		db1.close();
		...
		const db2 = new Database(dbPath);
		const after = db2.prepare("SELECT lease_until FROM tasks WHERE id = ?").get(taskId) as {
			lease_until: string;
		};
		db2.close();
	...
	it("does not mutate run when fencing token is stale", async () => {
		...
		const db1 = new Database(dbPath);
		const before = db1.prepare("SELECT last_heartbeat_at FROM runs WHERE id = ?").get(runId) as {
		...
		const db2 = new Database(dbPath);
		const after = db2.prepare("SELECT last_heartbeat_at FROM runs WHERE id = ?").get(runId) as {
			last_heartbeat_at: string | null;
		};
		db2.close();
	...
	it("skips writes when within throttle window (returns skipped:true)", async () => {
		...
		const db2 = new Database(dbPath);
		const after2 = db2.prepare("SELECT last_heartbeat_at FROM runs WHERE id = ?").get(runId) as {
			last_heartbeat_at: string;
		};
		db2.close();
	...
	it("does NOT throttle when hook is a lifecycle boundary", async () => {
		...
		const hook = db3.prepare("SELECT last_hook FROM runs WHERE id = ?").get(runId) as {
			last_hook: string;
		};
		db3.close();
