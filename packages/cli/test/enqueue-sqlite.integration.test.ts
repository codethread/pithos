...
describe("enqueueCommand (integration — real SQLite)", () => {
	...
	it("creates a queued task with a task_ prefixed ID", async () => {
		...
		const rows = db.prepare("SELECT id, status, capability FROM tasks").all() as {
			id: string;
			status: string;
			capability: string;
		}[];
	...
	it("stores title and body", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT title, body FROM tasks WHERE capability = 'triage'").get() as
	...
	it("reads body from --body-file when provided", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT body FROM tasks WHERE capability = 'watch'").get() as
			| { body: string }
			| undefined;
		db.close();
	...
	it("records created_by_run_id when --run is provided", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT created_by_run_id FROM tasks").get() as
...
describe("inspectTaskCommand (integration — real SQLite)", () => {
	...
	it("fails NOT_FOUND for an unknown task ID", async () => {
		const exit = await runEff(
			Effect.provide(inspectTaskCommand("task_ghost"), Layer.merge(dbLayer, silentOutput)),
		);
		expect(Exit.isFailure(exit)).toBe(true);
	});
});
