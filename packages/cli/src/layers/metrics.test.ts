...
describe("withCommandObservability", () => {
	...
	it("propagates a string error unchanged", async () => {
		...
		await expect(
			Effect.runPromise(Effect.fail(err).pipe(withCommandObservability("test-err"))),
...
describe("counter increments", () => {
	...
	for (const [name, counter] of counters) {
		it(`Metric.increment(${name}) runs without error`, async () => {
			await expect(Effect.runPromise(Metric.increment(counter))).resolves.toBeUndefined();
		});
	}
});
...
describe("commandDurationTimer", () => {
	it("trackDuration records duration without throwing", async () => {
		...
		await expect(
			Effect.runPromise(Effect.sleep("1 millis").pipe(Metric.trackDuration(tagged))),
	...
	it("tagged timer is transparent (result passes through)", async () => {
		...
		const result = await Effect.runPromise(Effect.succeed(99).pipe(Metric.trackDuration(tagged)));
