...
describe("supersedeCommand (integration — real SQLite)", () => {
  ...
  it("rewrites a -> b -> c to a -> d -> c, cancels queued old task, and exposes supersession links in inspect", async () => {
    ...
    setupDb
      ...
      .run()
    setupDb.prepare(`UPDATE tasks SET status = 'cancelled' WHERE id = 'task_cancelled_child'`).run()
    setupDb.close()
  ...
  it("fails USER_ERROR when the old task was already superseded", async () => {
    ...
    expect(Exit.isFailure(exit)).toBe(true)
    const cause = Exit.isFailure(exit) ? String(exit.cause) : ""
    expect(cause).toContain("Task task_old has already been superseded by task_existing_replacement")

    const postDb = new Database(dbPath)
    const taskIds = postDb.prepare(`SELECT id FROM tasks ORDER BY id ASC`).all() as { id: string }[]
    postDb.close()
  ...
  it("resets operational fields when superseding a failed task without cancelling it", async () => {
    ...
    setupDb
      .prepare(`UPDATE tasks SET payload_json = '{"copied":true}', max_attempts = 7 WHERE id = 'task_old'`)
      .run()
    setupDb.close()
    ...
    await Effect.runPromise(
      Effect.provide(
        failCommand({ taskId: "task_old", run: "run_worker", token: 1, reason: "bad intermediate output" }),
        Layer.merge(dbLayer, silentOutput),
      ),
    )
    ...
    const db = new Database(dbPath)
    const oldTask = db
      .prepare(`SELECT status, attempts, fencing_token, result_json FROM tasks WHERE id = 'task_old'`)
