...
export type HarnessOutput =
	| SpawnDescription
	| TmuxRunOutput
	| (TmuxRunOutput & { readonly session_file: string });
	...
	describe: (input) => {
		if (input.kind !== "claude") {
			throw new SpawnerError({
				...
				message: `claude harness received ${input.kind} config`,
			});
		}
		return {
			env: input.env,
...
const piHarness: HarnessAdapter = {
	...
	describe: (input) => {
		if (input.kind !== "pi") {
			throw new SpawnerError({
				code: "VALIDATION_ERROR",
				message: `pi harness received ${input.kind} config`,
			});
		}
...
const fakeHarness: HarnessAdapter = {
	name: "fake",
	describe: (input) =>
		input.kind === "claude" ? claudeHarness.describe(input) : piHarness.describe(input),
