...
describe("scopeUpsertCommand (integration — real SQLite)", () => {
	...
	beforeEach(async () => {
		...
		await Effect.runPromise(
			Effect.provide(initCommand, Layer.merge(makeDbServiceLive(dbPath), silentOutput)),
		);
	});
	...
	it("is idempotent — upserting the same path twice yields one row", async () => {
		...
		const db = new Database(dbPath);
		const rows = db.prepare("SELECT id FROM scopes WHERE id = ?").all(`repo:work/my-project`) as {
			id: string;
		}[];
		db.close();
	...
	it("upserts a global scope (no path required)", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT id, kind FROM scopes WHERE id = 'global'").get() as
	...
	it("expands ~ in --path correctly", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT id FROM scopes WHERE id = ?").get(expectedId) as
			| { id: string }
			| undefined;
		db.close();
...
describe("inspectScopeCommand (integration — real SQLite)", () => {
	...
	it("fails NOT_FOUND for unknown scope ID", async () => {
		const exit = await Effect.runPromiseExit(
			Effect.provide(
				inspectScopeCommand("repo:does-not-exist"),
				Layer.merge(makeDbServiceLive(dbPath), silentOutput),
			),
		);
	...
	it("returns the correct row after upsert", async () => {
		...
		const exit = await Effect.runPromiseExit(Effect.provide(inspectScopeCommand(id), layer));
		expect(Exit.isSuccess(exit)).toBe(true);
	});
});
