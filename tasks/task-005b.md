...
3. Create `<home>/runs/` for later AFK pidfiles.
4. Start the daemon in tmux target `pdx--daemon`.
5. Daemon binds the Unix socket and opens the Supervisor log.
6. Daemon upserts the `pdx` system run in global scope: `agent=pdx`, `mode=afk`, `scope=global`, `cwd=<home>`.
