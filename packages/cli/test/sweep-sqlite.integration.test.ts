...
describe("sweepCommand (integration — real SQLite)", () => {
  ...
  it("appends task.requeued event on requeue", async () => {
    ...
    const db = new Database(dbPath)
    const event = db
      .prepare("SELECT type, payload_json FROM events WHERE type = 'task.requeued' AND task_id = 'task_req_ev'")
  ...
  it("requeues an expired running task (status=running)", async () => {
    ...
    const row = db2
      .prepare("SELECT status FROM tasks WHERE id = 'task_running1'")
  ...
  it("dead-letters an expired task when attempts >= max_attempts", async () => {
    ...
    const db2 = new Database(dbPath)
    const row = db2
      .prepare("SELECT status FROM tasks WHERE id = 'task_dl1'")
      .get() as { status: string }
    db2.close()
  ...
  it("marks a run stale when heartbeat is older than run-stale-minutes", async () => {
    ...
    await Effect.runPromise(
      Effect.provide(
        sweepCommand({ runStaleMinutes: 15 }),
    ...
    const db = new Database(dbPath)
    const row = db
      .prepare("SELECT status FROM runs WHERE id = 'run_stale1'")
      .get() as { status: string }
    db.close()
  ...
  it("does not mark a run stale when heartbeat is within the threshold", async () => {
    ...
    const db = new Database(dbPath)
    const row = db
      .prepare("SELECT status FROM runs WHERE id = 'run_fresh1'")
      .get() as { status: string }
    db.close()
  ...
  it("marks a run stale when created_at is old and no heartbeat was recorded", async () => {
    ...
    const db2 = new Database(dbPath)
    const row = db2
      .prepare("SELECT status FROM runs WHERE id = 'run_no_hb'")
      .get() as { status: string }
    db2.close()
  ...
  it("does not affect already-ended runs", async () => {
    ...
    const db2 = new Database(dbPath)
    const row = db2
      .prepare("SELECT status FROM runs WHERE id = 'run_ended1'")
      .get() as { status: string }
    db2.close()
  ...
  it("outputs JSON with ok:true and zero counts when nothing to sweep", async () => {
    const out = makeOutputServiceTest()
    await Effect.runPromise(
      Effect.provide(sweepCommand(), Layer.merge(dbLayer, out.layer)),
    )
  ...
  it("does not requeue tasks whose lease_until is in the future", async () => {
    ...
    const db = new Database(dbPath)
    const row = db
      .prepare("SELECT status FROM tasks WHERE id = 'task_future'")
      .get() as { status: string }
    db.close()
  ...
  it("does not touch queued tasks (no lease)", async () => {
    ...
    const db = new Database(dbPath)
    const row = db
      .prepare("SELECT status FROM tasks WHERE id = 'task_queued1'")
      .get() as { status: string }
    db.close()
  ...
  it("respects --lease-grace-seconds: does not requeue when within grace period", async () => {
    ...
    await Effect.runPromise(
      Effect.provide(
        sweepCommand({ leaseGraceSeconds: 30 }),
        Layer.merge(dbLayer, silentOutput),
      ),
    )
    ...
    const db = new Database(dbPath)
    const row = db
      .prepare("SELECT status FROM tasks WHERE id = 'task_grace1'")
      .get() as { status: string }
    db.close()
  ...
  it("requeues when expiry is past the grace period", async () => {
    ...
    await Effect.runPromise(
      Effect.provide(
        sweepCommand({ leaseGraceSeconds: 10 }),
        Layer.merge(dbLayer, silentOutput),
      ),
    )
    ...
    const db = new Database(dbPath)
    const row = db
      .prepare("SELECT status FROM tasks WHERE id = 'task_grace2'")
      .get() as { status: string }
    db.close()
