...
| **Escalation task**       | A normal global-scope `escalate` capability task that routes attention to Pandora.                                                                                                                                                                                                                                           | Escalated status, blocked task, inbox message                                   |
...
| **Repair Alert**          | A system-authored Alert with a typed `kind` indicating the cause of broken/blocked work. Kinds: `interrupt`, `task_failed`, `dead_letter`, `launch_precondition`, `reconciler_stuck`, `kill_failure`. Always authored by `run_pdx_system`. Kinds that name a specific affected task also carry a `repair_source` link to it. | interruption escalation, launch-precondition escalation, system escalation      |
...
- All **Escalation tasks** live in global scope. A **Checkpoint escalation** depends on successful prior work; a **Repair Alert** references broken/failed work without depending on it.
...
- "Interruption escalation" and "Launch-precondition escalation" were separate UL nouns for specific Alert subtypes; retire both and use **Repair Alert** with a typed `kind` (`interrupt`, `task_failed`, `dead_letter`, `launch_precondition`, `reconciler_stuck`, `kill_failure`) instead. "System escalation" is also retired.
