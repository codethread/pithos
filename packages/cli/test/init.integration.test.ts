...
describe("initCommand (integration — real SQLite)", () => {
	...
	it("inserts the default global scope", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT id, kind, name FROM scopes WHERE id = 'global'").get() as
	...
	it("is idempotent — running init twice leaves both migration rows and one global scope", async () => {
		...
		const db = new Database(dbPath);
		const migRows = db.prepare("SELECT version FROM schema_migrations").all() as {
			version: number;
		}[];
		const scopeRows = db.prepare("SELECT id FROM scopes WHERE id = 'global'").all() as {
			id: string;
		}[];
		db.close();
	...
	it("does not touch ~/.pandora/pithos.sqlite", async () => {
		...
		const mtimeBefore: number | null = existsSync(realDbPath) ? statSync(realDbPath).mtimeMs : null;

		await runWith(initCommand, Layer.merge(makeDbServiceLive(dbPath), silentOutput));

		const mtimeAfter: number | null = existsSync(realDbPath) ? statSync(realDbPath).mtimeMs : null;
