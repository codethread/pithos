...
const runEnd = Command.make(
	"end",
	{
		run: Options.text("run").pipe(Options.optional, Options.withDescription("Run ID to end")),
...
const supersede = Command.make(
	"supersede",
	{
		...
		body: Options.text("body").pipe(
			Options.optional,
			Options.withDescription(
				"Inline replacement task body text (mutually exclusive with --body-file)",
			),
		),
		bodyFile: Options.text("body-file").pipe(
			Options.optional,
...
const inspectScope = Command.make("scope", { id: Args.text({ name: "scope-id" }) }, ({ id }) =>
	inspectScopeCommand(id),
).pipe(
	Command.withDescription(
		desc(
...
const inspectRun = Command.make("run", { id: Args.text({ name: "run-id" }) }, ({ id }) =>
	inspectRunCommand(id),
).pipe(
	Command.withDescription(
		desc(
			"Inspect a run by ID",
...
const inspectTask = Command.make("task", { id: Args.text({ name: "task-id" }) }, ({ id }) =>
...
const inspectGraph = Command.make(
	"graph",
	{
		...
		current: Options.boolean("current").pipe(Options.withDescription("Deprecated alias for --all")),
		flat: Options.boolean("flat").pipe(
			Options.withDescription(
				"Render a plain-text tree (opt-in text mode; hides completed chains by default)",
			),
		),
		dump: Options.boolean("dump").pipe(
			Options.withDescription(
				"Show all chains including completed ones (only meaningful with --flat)",
			),
		),
	},
...
const inspect = Command.make("inspect").pipe(
	Command.withDescription(
		HelpDoc.blocks([
			...
			HelpDoc.p(
				"Task inspection includes direct dependencies, direct dependents, unresolved blockers, and immediate supersession links.",
			),
			HelpDoc.p(
				"Graph inspection returns a closed transitive dependency/supersession graph for one task, one scope, or all non-cancelled work.",
			),
			HelpDoc.p("Examples:"),
...
const tail = Command.make(
	...
	Command.withDescription(
		HelpDoc.blocks([
			...
			HelpDoc.p(
				"  pithos tail --limit 100  # inspect task.created/task.cancelled/task.superseded payloads",
			),
...
const sweep = Command.make(
	...
	Command.withDescription(
		desc(
			...
			"pithos sweep",
			["pithos sweep", "pithos sweep --lease-grace-seconds 30 --run-stale-minutes 60"],
