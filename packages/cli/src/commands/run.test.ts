...
describe("runRegisterCommand (unit — fake DB)", () => {
	it("fails VALIDATION_ERROR when --agent-kind is missing", async () => {
		...
		const exit = await runEff(Effect.provide(runRegisterCommand({ agentKind: undefined }), layer));
	...
	it("succeeds when agent-kind is provided", async () => {
		...
		const exit = await runEff(Effect.provide(runRegisterCommand({ agentKind: "envy" }), layer));
