...
export const heartbeatCommand = (
	...
	Effect.gen(function* () {
					...
					const run = yield* Schema.decodeUnknown(RunRow)(runRows[0]!).pipe(
						Effect.mapError(
							() =>
								new PithosError({
									code: "INTERNAL_ERROR",
									message: "RunRow shape violation from DB",
								}),
						),
					);
		...
		yield* Metric.increment(heartbeatsWrittenCounter);
		yield* Effect.logDebug("heartbeat written").pipe(Effect.annotateLogs({ runId }));
		yield* output.print(JSON.stringify({ ok: true, skipped: false, run }));
	}).pipe(Effect.withLogSpan("pithos.heartbeat"), withCommandObservability("heartbeat"));
