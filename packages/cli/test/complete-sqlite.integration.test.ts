/**
 * Integration tests for pithos completeCommand and failCommand — real SQLite. Unit coverage lives in src/commands/complete.test.ts.
 */
...
describe("completeCommand (integration — real SQLite)", () => {
	...
	it("completes a claimed task and sets status to done", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT status, completed_at FROM tasks WHERE id = ?").get(taskId) as {
			status: string;
	...
	it("stores result_json from --result-file", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT result_json FROM tasks WHERE id = ?").get(taskId) as {
			result_json: string;
		};
		db.close();
	...
	it("appends a task.completed event", async () => {
		...
		const db = new Database(dbPath);
		const event = db
			.prepare("SELECT type, actor_run_id, payload_json FROM events WHERE type = 'task.completed'")
...
describe("failCommand (integration — real SQLite)", () => {
	...
	it("fails a claimed task and sets status to failed", async () => {
		...
		const db = new Database(dbPath);
		const row = db.prepare("SELECT status, result_json FROM tasks WHERE id = ?").get(taskId) as {
			status: string;
			result_json: string;
		};
		db.close();
	...
	it("appends a task.failed event", async () => {
		...
		const db = new Database(dbPath);
		const event = db
			.prepare("SELECT type, actor_run_id, payload_json FROM events WHERE type = 'task.failed'")
			.get() as { type: string; actor_run_id: string; payload_json: string };
		db.close();
