...
| **Task chain**            | The work thread the user and agents discuss: the delegation path from initial triage through design, escalation/review, execution, result review, and repair.                                                                                                                                                                                                         | Task graph, pipeline if it implies fixed stages, persisted chain object         |
...
| **Source link**           | A non-blocking provenance edge meaning this task is about, or came from, a source task. It helps reconstruct the chain but does not affect claimability. Source links may be normal continuation provenance or repair provenance for broken work.                                                                                                                     | relates_to, dependency, parent, blocker                                         |
...
| **Review task**           | A `review` Capability Task claimed by Greed for requested HITL assessment. It may be global, repo, or worktree scoped; normally it is explicitly requested and dependency-gated like ordinary follow-up work.                                                                                                                                                         | Escalation, approval status, automatic gate                                     |
| **Escalation task**       | A normal global-scope `escalate` capability task that routes immediate attention, repair, and routing to Pandora.                                                                                                                                                                                                                                                     | Escalated status, blocked task, inbox message, review task                      |
...
| **Repair Alert**          | A system-authored Alert with a typed `kind` indicating the cause of broken/blocked work. Kinds: `interrupt`, `task_failed`, `dead_letter`, `launch_precondition`, `reconciler_stuck`, `kill_failure`, `input_hook_stuck`, `hook_config_error`. Always authored by `run_pdx_system`. Kinds that name a specific affected task also carry a `repair_source` link to it. | interruption escalation, launch-precondition escalation, system escalation      |
...
| **Alert**         | A system-authored durable record that the control plane or durable state has hit an anomaly needing human attention. Implemented as a global-scope `escalate` task authored by the pdx system actor. Distinguished from agent-filed escalations because no agent can file it — the supervisor or engine is the author. Durable, survives pdx restart, recoverable from the task graph.                                                                                                                       | System escalation, harness alert                                  |
| **Nudge**         | A best-effort, content-free signal from the control plane to a live harness session via the **Control-plane backend** (today: `tmux send-keys`). Carries no payload; the recipient consults durable Pithos state. Always paired with a durable Pithos artifact (a claimable escalation task or an **Alert**); the Nudge only shortens the time until the harness notices. Not retried to success; lost if the harness is unreachable. The trigger is typed via a `reason` field (e.g. `claimable_escalate`). | Wakeup (retired), message injection, nudge-with-body, tmux marker |
...
- All **Escalation tasks** live in global scope. A **Review task** is ordinary non-escalation work claimed by **Greed** and may live in global, repo, or worktree scope. A **Checkpoint escalation** depends on successful prior work; a **Repair Alert** references broken/failed work without depending on it.
- A normal attention **Escalation task** may carry a continuation **Source link** to the task it is about so **Pandora** can route the chain onward without making the escalation wait on that source.
- A **Repair Alert** carries a `repair_source` **Source link** to broken work when it names an affected task; **Pandora** should repair it with **Supersession** or explicit replanning, not ordinary chain continuation.
...
- If **pdx** finds a **Claimable task** whose repo/worktree runtime path no longer exists before run creation, no **Run** is created. If that path disappears after a run row is created but before launch succeeds, **pdx** terminalizes the no-claim **Run** before repairing the queued task. In both cases **pdx** uses **Cancel** on the queued task and creates a **Repair Alert** (kind=`launch_precondition`) for **Pandora** to repair with **Supersession** or replanning.

## Example dialogue

> **Dev:** "If **War** goes off the rails while holding an `execute` **Task**, do we just requeue the task?"
>
> **Domain expert:** "No. A deliberate **Kill** uses **Interrupt**, so the **Held task** becomes `failed`, its **Fencing token** is incremented, and the engine creates a **Repair Alert** (kind=`interrupt`) for **Pandora**. Use **Interrupt** because a live **Run** is involved."
>
...
>
> **Domain expert:** "No. A **Dependency** only unblocks on `done`, so a **Repair Alert** references the failed task without depending on it."
>
> **Dev:** "What if a repo directory vanished before War launched?"
>
> **Domain expert:** "No one held the task yet, so **pdx** does not mark it `failed`. It uses **Cancel** on the queued task, creates a **Repair Alert** (kind=`launch_precondition`), and Pandora repairs the **Broken chain** by recreating/upserting the scope and superseding the cancelled task or replanning."
>
...
- "Wakeup" was previously used as a noun for the harness-side effect of a content-free nudge; retire **Wakeup** and use **Nudge** plus a typed `reason` field instead. "Nudge" previously meant prompt/message injection in early drafts; that meaning is also retired.
- "Launch failure" can mean invalid scope runtime path or harness/config/process failure. Use **Repair Alert** (kind=`launch_precondition`) only for the former; harness/config/process failures remain supervisor errors and must not silently cancel user work.
...
- "Interruption escalation" and "Launch-precondition escalation" were separate UL nouns for specific Alert subtypes; retire both and use **Repair Alert** with a typed `kind` (`interrupt`, `task_failed`, `dead_letter`, `launch_precondition`, `reconciler_stuck`, `kill_failure`, `input_hook_stuck`, `hook_config_error`) instead. "System escalation" is also retired.
