...
| `hook_config_error`   | pdx failed to load the input hook config from `agents.json`; the input hook is off.    | Show the alert body to the user. Ask them to fix the agents.json manifest overlay (usually `$PDX_DATA_DIR/extensions/templates/agents.json` for custom hooks), then restart pdx (`pdx close && pdx open`) to re-enable hook supervision. |
