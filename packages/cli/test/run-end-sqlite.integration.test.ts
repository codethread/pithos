...
describe("runEndCommand (integration — real SQLite)", () => {
	...
	beforeEach(async () => {
		...
		await Effect.runPromise(
			Effect.provide(runRegisterCommand({ agentKind: "envy", run: "run_to_end" }), layer),
		);
	});
	...
	it("sets ended_at on a run when ended", async () => {
		...
		const row = db.prepare("SELECT status, ended_at FROM runs WHERE id = 'run_to_end'").get() as
	...
	it("is idempotent — ending an already-ended run returns success without re-appending an event", async () => {
		...
		const db = new Database(dbPath);
		const events = db.prepare("SELECT type FROM events WHERE type = 'run.ended'").all() as {
			type: string;
		}[];
		db.close();
		expect(events).toHaveLength(1);
	...
	it("supports cancellation status", async () => {
		...
		const db = new Database(dbPath);
		const cancelRow = db.prepare("SELECT status FROM runs WHERE id = 'run_to_end'").get() as
			| { status: string }
			| undefined;
		db.close();
