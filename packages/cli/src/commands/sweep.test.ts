...
describe("sweepCommand (unit — fake DB)", () => {
	...
	it("fails VALIDATION_ERROR when --lease-grace-seconds is negative", async () => {
		const exit = await runEff(Effect.provide(sweepCommand({ leaseGraceSeconds: -1 }), fakeLayer));
		expect(Exit.isFailure(exit)).toBe(true);
	});

	it("fails VALIDATION_ERROR when --run-stale-minutes is zero", async () => {
		const exit = await runEff(Effect.provide(sweepCommand({ runStaleMinutes: 0 }), fakeLayer));
		expect(Exit.isFailure(exit)).toBe(true);
	});

	it("fails VALIDATION_ERROR when --run-stale-minutes is negative", async () => {
		const exit = await runEff(Effect.provide(sweepCommand({ runStaleMinutes: -5 }), fakeLayer));
		expect(Exit.isFailure(exit)).toBe(true);
	});

	it("accepts --lease-grace-seconds of 0 (default)", async () => {
		const exit = await runEff(Effect.provide(sweepCommand({ leaseGraceSeconds: 0 }), fakeLayer));
		expect(Exit.isSuccess(exit)).toBe(true);
	});
});
